	...
	if (temp == NULL) {
		...
		if (temp->left == NULL && temp->right == NULL) {
			...
			if (temp->right != NULL) {
				...
				// key값이 있는 노드에 자식노드가 있고, 그 자식노드의 left자식만 있는 경우
				previous = temp;
				// previous의 위치를 temp로 이동
				temp = temp->left;
				// temp는 left로 이동
				previous->key = temp->key;
				// 왼쪽노드에서 가장 처음에 있는 값을 옮긴다음
				// 그에대한 left와 right의 노드를 각각 previous의 left와 right에 옮긴다
				// 우리가 찾고자 하는 key값이 있는 노드에 right자식이 없는 경우이므로 아래의 코드 가능
